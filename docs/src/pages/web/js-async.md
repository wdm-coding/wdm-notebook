...
::: warning for...in 与 for...of的区别
  1. for...in 遍历对象的可枚举属性（包括原型链上的属性） 返回的是属性的键（key），而不是值
  2. for...of 遍历可迭代对象（如数组、字符串、Map、Set等）的元素值 属性的值（value），而不是键
  3. 当使用 for...in 循环遍历对象时，会返回对象自身和继承的可枚举属性 使用 hasOwnProperty 可以过滤掉继承的属性，只处理对象自身的属性
  (ES6+ 中，可以使用 Object.hasOwn(obj,key) 方法)
:::
```js
<script>
  function muti(num){
