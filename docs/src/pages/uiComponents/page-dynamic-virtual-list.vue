...
const resizeObserver = ref(null) // 用于监听容器尺寸变化的ResizeObserver实例
// 总高度计算 = 最后一个元素底部位置
...
const isScrolledToBottom = () => {
  if (!containerRef.value) return false
   ...
   const isBottom = scrollHeight - (scrollTop + clientHeight) < 50 // 50px的缓冲区
   return isBottom
}
// 滚动处理
const handleScroll = () => {
...
const initObserver = () => {
  ...
  resizeObserver.value = new ResizeObserver(entries => {
    entries.forEach(entry => {
      ...
      if (measuredHeights.value.get(id) !== height) {
...
// 模拟分页获取数据
const fetchData = () => {
  list.value = list.value.concat(Array.from({ length: 10 }, (_, i) => ({
    id: `item-${list.value.length + i}`,
    content: `Item ${list.value.length + i}`,
